...
def test1():
    ...
    check50.run("./credit").stdin("31406215").stdout(r"^\s*VALID\s*\n", regex=True).stdout(check50.EOF).exit(0)

@check50.check(compiles)
def test2():
    """identifies 00000000 as VALID"""
    check50.run("./credit").stdin("00000000").stdout(r"^\s*VALID\s*\n", regex=True).stdout(check50.EOF).exit(0)
...
def test5():
    ...
    check50.run("./credit").stdin("12341214").stdout(r"^\s*VALID\s*\n", regex=True).stdout(check50.EOF).exit(0)

@check50.check(compiles)
def test6():
    """identifies 56785678 as VALID"""
    check50.run("./credit").stdin("56785678").stdout(r"^\s*VALID\s*\n", regex=True).stdout(check50.EOF).exit(0)

@check50.check(compiles)
def test7():
    """identifies 51785678 as VALID"""
    check50.run("./credit").stdin("51785678").stdout(r"^\s*VALID\s*\n", regex=True).stdout(check50.EOF).exit(0)
