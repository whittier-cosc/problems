...
def test_reject_negative():
    ...
    check50.run("./triangle").stdin("-1").reject()

@check50.check(compiles)
def test0():
    """rejects a height of 0"""
    check50.run("./triangle").stdin("0").reject()

@check50.check(compiles)
def test1():
    """handles a height of 1 correctly"""
    out = check50.run("./triangle").stdin("1").stdout()
    check_pyramid(out, open("1.txt").read())

@check50.check(compiles)
def test2():
    """handles a height of 2 correctly"""
    out = check50.run("./triangle").stdin("2").stdout()
    check_pyramid(out, open("2.txt").read())

@check50.check(compiles)
def test23():
    """handles a height of 8 correctly"""
    out = check50.run("./triangle").stdin("8").stdout()
    check_pyramid(out, open("8.txt").read())

@check50.check(compiles)
def test24():
    """rejects a height of 11, and then accepts a height of 2"""
    (check50.run("./triangle").stdin("11").reject()
            .stdin("2").stdout(open("2.txt")).exit(0))

@check50.check(compiles)
def test_reject_foo():
    """rejects a non-numeric height of "foo" """
    check50.run("./triangle").stdin("foo").reject()
